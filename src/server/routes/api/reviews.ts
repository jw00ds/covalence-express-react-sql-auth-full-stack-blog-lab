...
router.post("/", passport.authenticate("jwt"), async (req: ReqUser, res) => {
  ...
  try {
    const reviewInsertRes = await db.reviews.createReview(
      newFormEntry.title,
      newFormEntry.commentary,
      newFormEntry.image_url,
      req.user.id
    );
    // const reviewInsertRes = await db.reviews.createReview(newFormEntry);
    if (newFormEntry.tagid) {
      await db.reviewtags.createReviewTag(
        reviewInsertRes.insertId,
        newFormEntry.tagid
      );
    }
...
router.put("/:id", passport.authenticate("jwt"), async (req, res) => {
  ...
  try {
    const reviewRes = await db.reviews.reviseReview(
      revisedReview.title,
      revisedReview.commentary,
      revisedReview.image_url,
      id
    );
