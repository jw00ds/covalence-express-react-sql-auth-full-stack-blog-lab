...
export default function CreateReview() {
  ...
  const handleCommentaryInput = (e: React.ChangeEvent<HTMLTextAreaElement>) =>
    setCommentary(e.target.value);

  const handleTagInput = (e: React.ChangeEvent<HTMLSelectElement>) =>
  ...
  const createReview = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    e.preventDefault();
    // No need for riderid below, b/c applied in reviews router
    let newReview = {
      title,
      commentary,
      image_url: img,
    ...
    // if (selectedTag !== '0') {
    //     const res = await fetch('/api/reviewtags', {
    //         method: 'POST',
    //         headers: {
    //             'Content-Type': 'application/json'
    //         },
    //         body: JSON.stringify({ reviewid: reviewResult.id, tagid: selectedTag });
    //     });
    // }
  };
        ...
        <div className="col-md-9 m-2">
          ...
          <form className="form-group">
            ...
            >
              {tagsList.map((tag) => (
                <option
                  key={`tagoption-${tag.Review_Tag_ID}`}
                  value={tag.Review_Tag_ID}
                >
                  #{tag.Review_Tag}
                </option>
              ))}
            ...
            <br />
            <button className="btn btn-warning" onClick={createReview}>
              Post
            </button>
          </form>
        </div>
